...
fn lcs(seq1: &Vec<String>, seq2: &Vec<String>) -> Grid {
    ...
    let len_1 = seq1.len();
    let len_2 = seq2.len();
    let mut martix = Grid::new(len_1 + 1, len_2 + 1);
    for i in 0..len_1 + 1 {
        martix.set(i, 0, 0);
    }
    for i in 0..len_2 + 1 {
    ...
    for i in 0..len_1 {
        for j in 0..len_2 {
            if seq1[i] == seq2[j] {
                martix.set(i + 1, j + 1, martix.get(i, j).unwrap() + 1);
            } else {
                martix.set(
                    i + 1,
                    j + 1,
                    cmp::max(martix.get(i, j + 1).unwrap(), martix.get(i + 1, j).unwrap()),
                );
            }
        }
    }
...
fn main() {
    ...
    // unimplemented!();
    // Be sure to delete the #[allow(unused)] line above
    let text1 = read_file_lines(filename1).unwrap();
    let text2 = read_file_lines(filename2).unwrap();
    let martix = lcs(&text1, &text2);
    let len1 = text1.len();
    let len2 = text2.len();
    martix.display();
    print_diff(&martix, &text1, &text2, len1, len2);
}
